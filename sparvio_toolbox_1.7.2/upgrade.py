...
def get_latest_fw_version(hw_model, hw_rev, app_id):
    ...
    if hw_model.lower() == 'skh1':
        long_name = app_ids.id_to_long_name(app_id)
        if long_name in ['S2', 'S2S', 'S2R']:
            # Since S2 uses the SKH1 bootloader, we need to check if it's actually S2
            hw_model = 's2'
        elif long_name == 'S2 H4':
            hw_model = 's2h4'
...
def main_sa1(args, sa1):
    ...
    if args.writelatest:
        ...
        if new_appRev is None or url is None:
